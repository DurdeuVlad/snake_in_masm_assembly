...
void moveSnake(Snake* snake, Point new_pos){
    for(int i= snake->current_length - 1; i > 0; i--){
...
//    if(snake->current_length == MAX*2-1)
//        return;
    Point new_pos;
    switch (snake->direction) {
        case 'd':
            new_pos.x = snake->body[snake->current_length - 1].x;
            new_pos.y = snake->body[snake->current_length - 1].y - 1;
            break;
        case 'a':
            new_pos.x = snake->body[snake->current_length - 1].x;
            new_pos.y = snake->body[snake->current_length - 1].y + 1;
            break;
        case 's':
            new_pos.x = snake->body[snake->current_length - 1].x - 1;
            new_pos.y = snake->body[snake->current_length - 1].y;
            break;
        case 'w':
            new_pos.x = snake->body[snake->current_length - 1].x + 1;
            new_pos.y = snake->body[snake->current_length - 1].y;
    ...
    snake->body = (Point*)realloc(snake->body, snake->current_length * sizeof(Point));
    snake->body[snake->current_length - 1] = new_pos;
}
...
int reactToScreenPositionAndDrawSnake(int screen[MAX][MAX], Snake* snake){
    ...
    if(screen[current_pos.x][current_pos.y] == 1 || screen[current_pos.x][current_pos.y] == 2){
        ...
        int i, overlaps = 1;
        for(i= snake->current_length - 1; i > 1; i--){
